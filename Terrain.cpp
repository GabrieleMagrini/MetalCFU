 ...
 */
    if ((this->getLocalBounds().width + this->getPosition().x - g->getPosition().x) >= 0 &&
        (this->getLocalBounds().width + this->getPosition().x - g->getPosition().x) <= this->getLocalBounds().width) {

        if ((this->getPosition().y - g->getPosition().y) >= 0 &&
            (this->getPosition().y - g->getPosition().y) <=
            g->getLocalBounds().height) {
            if (g->getSpeedY() != 0) {

                g->setCollisionY(true);
                g->setPosition(g->getPosition().x, this->getPosition().y - g->getLocalBounds().height);
            } else if (g->getSpeedX() != 0)
                g->setCollisionX(Collision::PixelPerfectTest(*this, *g));
            else {
                g->setCollisionY(false);
            }
        }
    }
